	...
	$wp_customize->add_control(new StoreOne_Info_Text($wp_customize, 'storeone_homepage_setup',
		array(
			...
			'description' => __('1. Create or Edit page with name Home -> Select Template "Home Page" -> Publish. <br><br>
    							2. Go To Appearance -> Customize -> Static Front Page -> Front page displays set it to "A static page" -> for Front page select Home. <a class="storeone_go_to_section" href="#accordion-section-static_front_page"> <strong>Switch To "A Static Page" </strong></a>', 'storeone'),
			'priority' => 1,
...
/* Other Options */
	...
	if(function_exists('storeone_extension')){
		...
		$wp_customize->add_setting('storeone_add_slide_button', array(
			'sanitize_callback' => 'storeone_sanitize_html',
		));

		$wp_customize->add_control(new StoreOne_Info_Text($wp_customize, 'storeone_add_slide_button',
			array(
				'label'    => __('Add Slides', 'storeone'),
				'description' => sprintf('<a class="button button-default" href="%1$s" target="_blank">%2$s</a>', esc_url(admin_url('edit.php?post_type=tf_slider')), esc_html__('Add Slides', 'storeone')),
				'priority' => 999,
				'section'  => 'storeone_home_slider_section',
		)));

		$wp_customize->add_setting('storeone_add_testimonial_button', array(
			'sanitize_callback' => 'storeone_sanitize_html',
		));

		$wp_customize->add_control(new StoreOne_Info_Text($wp_customize, 'storeone_add_testimonial_button',
			array(
				'label'    => __('Add Testimonials', 'storeone'),
				'description' => sprintf('<a class="button button-default" href="%1$s" target="_blank">%2$s</a>', esc_url(admin_url('edit.php?post_type=tf_testimonials')), esc_html__('Add Testimonials', 'storeone')),
				'priority' => 999,
				'section'  => 'storeone_home_testimonial_section',
		)));		
	}
